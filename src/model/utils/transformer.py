...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.attn_mode == 'Efficient':
            ...
            A = self.memory_efficient_attention(
                Q_, K_, V_, q_bucket_size=q_bucket_size, k_bucket_size=k_bucket_size
            )  # [B, H, N, Dh]

            # Keep batch dimension intact when merging heads.
            O = (Q_ + A).permute(0, 2, 1, 3).reshape(Q.shape[0], Q.shape[1], self.dim_V)
        else: # Basic
